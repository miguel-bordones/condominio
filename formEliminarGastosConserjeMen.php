<?php  
    ...
    $year = substr($GACM_FE_GASTO,0,4);
    $mes  = substr($GACM_FE_GASTO,5,2);
?>

    <main class="container">
        <h1>Eliminar Gasto Conserje Mensual</h1>

        <div class="alert bg-light p-4 col-6 mx-auto border border-danger shadow-sm">
            <div class="row">
        
                <div class="col text-danger">
                    <h2> Codigo de Gasto: <?=  $gasto['GACM_CD_GASTO'] ?></h2>
                         Desc. Gasto :    <?=  $gasto['GCON_DESC_GASTO'] ?><br>
                         Monto Gasto :    <?=  $gasto['GACM_MT_GASTO'] ?><br>
                         Fecha Gasto :    <?=  $gasto['GACM_FE_GASTO'] ?><br>
                         

                    <form action="eliminarGastosConserjeMen.php" method="post">
                        <input type="hidden" name="GACM_CD_GASTO"
                               value="<?= $gasto['GACM_CD_GASTO'] ?>">
                        <input type="hidden" name="GACM_FE_GASTO"
